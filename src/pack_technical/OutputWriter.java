...
public class OutputWriter {
    ...
    // plot polynomials
    static ArrayList<PrintWriter> POLY_separation_weight_writers = new ArrayList<PrintWriter>();
    ...
    public OutputWriter() {
        // parameter writers
        PARAM_separation_weight_writers.clear();
        ...
        // polynomial writers
        POLY_separation_weight_writers.clear();
        ...
        for (int observer = 0; observer <= GameManager.getTeam_number()+1; observer++) {
            for (int observed = 0; observed <= GameManager.getTeam_number()+1; observed++) {
                try {
                    ...
                    POLY_separation_weight_writers.add(swp);
                    writers.add(swp);
    ...
    public static void output_perspective(int observer_t, int observed_t, String value, String param) {
        ...
        switch (param) {
        case "param_sw":
            if (PARAM_separation_weight_writers.size() >= observer_t)
                PARAM_separation_weight_writers.get(index).println(value);
            break;
        case "param_aw":
        ...
        case "param_cns":
            ...
            // polynomial
        case "poly_sw":
            if (POLY_separation_weight_writers.size() >= observer_t)
                POLY_separation_weight_writers.get(index).println(value);
            break;
        case "poly_aw":
